...
fn emit_vnc_snapshot(
    ...
    let buf = match image::ImageBuffer::from_raw(desktop_width as u32, desktop_height as u32, snapshot_rgba.to_vec()) {
        Some(buf) => buf,
...
async fn handle_vnc_control(control: VncControlMsg, client: &VncClient) -> Result<VncControlOutcome, String> {
    match control {
